...
def splitMo2ModlistHeader(lines):
    ...
    MO2 stores ``modlist.txt`` in reverse UI priority order: file top is the
    bottom of the left pane and wins conflicts. Raw file edits should therefore
    use UI-named helpers instead of appending entries directly.
    """
    header = []
    body = []
    for line in lines:
...
def moveModlistEntriesToUiBottom(modlist_path, mod_names, backup_dir=None):
    ...
    This writes entries immediately after the generated header because MO2's
    on-disk order is inverted relative to the visible priority list.
    """
