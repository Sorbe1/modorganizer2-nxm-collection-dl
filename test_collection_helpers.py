...
from collection_helpers import (
    ...
    bethesdaPluginMastersFromBytes,
...
class ProfileStateAuditTests(unittest.TestCase):
    def test_parses_bethesda_plugin_masters_from_tes4_header(self):
        plugin_bytes = fakeBethesdaPluginBytes(["Skyrim.esm", "Update.esm"])

        self.assertEqual(
            bethesdaPluginMastersFromBytes(plugin_bytes),
            ["Skyrim.esm", "Update.esm"],
        )

    def test_parses_mo2_plugins_text_available_and_active_plugins(self):
        parsed = parseMo2PluginsText(
            "# This file was generated by Mod Organizer.\n"
            "*Skyrim.esm\n"
            "Disabled.esp\n"
            "*Patch.esp\n"
        )
        ...
        self.assertEqual(
            parsed,
            {
                "available": ["Skyrim.esm", "Disabled.esp", "Patch.esp"],
                "active": ["Skyrim.esm", "Patch.esp"],
            },
        )
    ...
    def test_reports_missing_master_from_managed_plugin_header(self):
        with TemporaryDirectory() as tmp:
            ...
            mod_dir = base / "mods" / "Patch Mod"
            mod_dir.mkdir(parents=True)
            (mod_dir / "meta.ini").write_text("[General]\n", encoding="utf-8")
            (mod_dir / "Patch.esp").write_bytes(
                fakeBethesdaPluginBytes(["Skyrim.esm", "Missing.esm", "Update.esm"])
            )

            result = auditMo2ProfileState(base_path=base)

            self.assertFalse(result["clean"])
            self.assertTrue(result["plugin_dependency_audit"]["supported"])
            self.assertEqual(result["plugin_dependency_audit"]["checked"], 1)
            self.assertEqual(result["plugin_dependency_audit"]["problem_count"], 1)
            self.assertEqual(
                result["plugin_dependency_audit"]["problems"][0]["missing_masters"],
                ["Missing.esm"],
            )
            self.assertIn(
                "plugin_master_dependencies",
                [issue["type"] for issue in result["issues"]],
            )

    def test_loadorder_plugins_satisfy_managed_plugin_masters(self):
        with TemporaryDirectory() as tmp:
            base = Path(tmp)
            profile = base / "profiles" / "Default"
            profile.mkdir(parents=True)
            (profile / "modlist.txt").write_text("+Patch Mod\n", encoding="utf-8")
            (profile / "plugins.txt").write_text("*Patch.esp\n", encoding="utf-8")
            (profile / "loadorder.txt").write_text(
                "Skyrim.esm\nUpdate.esm\nPatch.esp\n", encoding="utf-8"
            )
            mod_dir = base / "mods" / "Patch Mod"
            ...
            (mod_dir / "meta.ini").write_text("[General]\n", encoding="utf-8")
            (mod_dir / "Patch.esp").write_bytes(
                fakeBethesdaPluginBytes(["Skyrim.esm", "Update.esm"])
            )
            ...
            self.assertTrue(result["clean"])
            self.assertTrue(result["plugin_dependency_audit"]["supported"])
            self.assertEqual(result["plugin_dependency_audit"]["problem_count"], 0)

    def test_reports_inactive_master_from_managed_plugin_header(self):
        with TemporaryDirectory() as tmp:
            base = Path(tmp)
            profile = base / "profiles" / "Default"
            profile.mkdir(parents=True)
            ...
            (mod_dir / "meta.ini").write_text("[General]\n", encoding="utf-8")
            (mod_dir / "Patch.esp").write_bytes(
                fakeBethesdaPluginBytes(["Skyrim.esm", "Inactive.esm"])
            )
            ...
            self.assertEqual(result["plugin_dependency_audit"]["problem_count"], 1)
            self.assertEqual(
                result["plugin_dependency_audit"]["problems"][0]["inactive_masters"],
                ["Inactive.esm"],
            )
    ...
    def test_warns_for_likely_modlist_ordering_diagnostics(self):
        with TemporaryDirectory() as tmp:
            base = Path(tmp)
            profile = base / "profiles" / "Default"
            profile.mkdir(parents=True)
            (profile / "modlist.txt").write_text(
                "+Community Shaders\n"
                "+Community Shaders - Particle Lights Patch\n"
                "+DLC: Dragonborn\n"
                "+DLC: HearthFires\n"
                "+DLC: Dawnguard\n",
                encoding="utf-8",
            ...
            self.assertEqual(result["issues"], [])
            self.assertEqual(
                result["modlist_ordering_diagnostics"][0]["type"],
                "patch_before_target",
            )
            self.assertIn(
...
class MoveModlistEntriesToUiBottomTests(unittest.TestCase):
    def test_modlist_entries_parse_disk_order_as_reverse_visible_priority(self):
        modlist_text = (
            "# This file was automatically generated by Mod Organizer.\n"
            "+Visible Bottom Highest Priority\n"
            "+Visible Middle Priority\n"
            "+Visible Top Priority Zero\n"
        )

        entries = mo2ModlistEntries(modlist_text)

        self.assertEqual(
            [(entry["name"], entry["priority"]) for entry in entries],
            [
                ("Visible Top Priority Zero", 0),
                ("Visible Middle Priority", 1),
                ("Visible Bottom Highest Priority", 2),
            ],
        )

    def test_moves_entries_to_file_start_for_visible_ui_bottom(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            modlist = root / "modlist.txt"
            modlist.write_text(
    ...
    def test_base_modlist_order_accepts_managed_then_creation_club_then_dlc(self):
        modlist_text = (
            "# This file was automatically generated by Mod Organizer.\n"
            "+Managed Texture Pack\n"
    ...
    def test_base_modlist_order_detects_unmanaged_before_creation_club(self):
        modlist_text = (
            "# This file was automatically generated by Mod Organizer.\n"
            "+Creation Club: _ResourcePack\n"
            "+Unmanaged: hotrest nord presets\n"
    ...
    def test_modlist_ordering_diagnostics_reports_patch_before_target(self):
        modlist_text = (
            "# This file was automatically generated by Mod Organizer.\n"
            "+Community Shaders\n"
            "+Alternative Conversation Camera - Community Shaders patch\n"
        )

        diagnostics = mo2ModlistOrderingDiagnostics(modlist_text)

        self.assertEqual(diagnostics[0]["type"], "patch_before_target")
        self.assertEqual(
            diagnostics[0]["mod"],
            "Alternative Conversation Camera - Community Shaders patch",
        )
        self.assertEqual(diagnostics[0]["target"], "Community Shaders")

    def test_modlist_ordering_diagnostics_ignores_variant_without_file_evidence(self):
        modlist_text = (
            "# This file was automatically generated by Mod Organizer.\n"
        ...
        self.assertEqual(mo2ModlistOrderingDiagnostics(modlist_text), [])

    def test_modlist_ordering_diagnostics_reports_variant_with_shared_files(self):
        with TemporaryDirectory() as tmp:
            mods = Path(tmp)
            variant = (
                mods
            ...
            (base / "textures" / "rain" / "drop.dds").write_bytes(b"base")
            modlist_text = (
                "# This file was automatically generated by Mod Organizer.\n"
                "+True Storms Special Edition - Thunder Rain and Weather Redone\n"
                "+True Storms Special Edition - Thunder Rain and Weather Redone #2\n"
        ...
        self.assertEqual(diagnostics[0]["type"], "variant_before_base")
        self.assertEqual(
            diagnostics[0]["mod"],
            "True Storms Special Edition - Thunder Rain and Weather Redone #2",
        )
        self.assertEqual(diagnostics[0]["shared_file_count"], 1)
        self.assertEqual(
            diagnostics[0]["shared_file_examples"],
            ["textures/rain/drop.dds"],
        )

    def test_modlist_ordering_diagnostics_ignores_ordered_patch(self):
        modlist_text = (
            "# This file was automatically generated by Mod Organizer.\n"
            "+Alternative Conversation Camera - Community Shaders patch\n"
            "+Community Shaders\n"
    ...
    def test_repairs_patch_modlist_ordering_diagnostic_after_target(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            backup = root / "backup"
            modlist = root / "modlist.txt"
            ...
            result = repairMo2ModlistOrderingDiagnostics(
                modlist,
                backup_dir=backup,
            )
    ...
    def test_repairs_variant_modlist_ordering_diagnostic_after_base(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            mods = root / "mods"
            variant = mods / "Vanilla hair remake #2"
            base = mods / "Vanilla hair remake"
            (variant / "meshes").mkdir(parents=True)
            (base / "meshes").mkdir(parents=True)
            (variant / "meshes" / "hair.nif").write_bytes(b"variant")
            (base / "meshes" / "hair.nif").write_bytes(b"base")
            modlist = root / "modlist.txt"
            modlist.write_text(
                "# This file was automatically generated by Mod Organizer.\n"
                "+Vanilla hair remake\n"
                "+Other Mod\n"
                "+Vanilla hair remake #2\n",
                encoding="utf-8",
            )
    ...
    def test_base_modlist_order_repair_handles_starred_unmanaged_entries(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            backup = root / "backup"
            modlist = root / "modlist.txt"
    ...
    def test_base_modlist_order_repair_places_unmanaged_before_managed(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            modlist = root / "modlist.txt"
            modlist.write_text(
...
class DownloadTailBoundaryTests(unittest.TestCase):
    ...
    def test_adaptive_submission_limit_queues_moderate_collection(self):
        self.assertEqual(adaptiveDownloadQueueSubmissionLimit(53, 16), 53)

    def test_adaptive_submission_limit_caps_large_collection(self):
        self.assertEqual(adaptiveDownloadQueueSubmissionLimit(559, 16), 64)
...
class InstallRuntimeSourceTests(unittest.TestCase):
    def install_source(self):
        return (Path(__file__).resolve().parent / "install.py").read_text(
            encoding="utf-8"
        )
